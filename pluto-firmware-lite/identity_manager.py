...
class IdentityManager:
    def __init__(self, path: str = IDENTITY_FILE, x25519_path: str = X25519_TEMP_FILE):
        ...
        self._volatile_identity = None
        self._volatile_x25519 = None

    def ensure_identity(self):
        if self._volatile_identity:
            seed, pub = self._volatile_identity
    ...
    def clear_x25519_temporary(self) -> bool:
        if self._volatile_x25519 is not None:
            self._volatile_x25519 = None
            return True
        try:
    ...
    def _save_identity(self, seed: bytes, pub: bytes):
        ...
        try:
            ...
            # Read-only storage fallback: keep identity in RAM for this boot.
            if getattr(exc, "errno", None) == 30:
                self._volatile_identity = (seed, pub)
                return
            raise
