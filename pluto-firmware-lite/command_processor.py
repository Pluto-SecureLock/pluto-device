    ...
    def execute(self, command):
        ...
        if command.startswith("encrypt "):
            ...
            try:
                if not self.identity:
                    raise RuntimeError("Identity manager not initialized")
                x25519_pub = self.identity.generate_x25519_public_key_hex()
                self.secure_write(f"{x25519_pub}\n")
            except Exception as e:
                self.secure_write(f"❌ Failed to generate x25519 keypair: {e}\n")
        elif command == "x25519_clear":
            try:
                if not self.identity:
                    raise RuntimeError("Identity manager not initialized")
                if self.identity.clear_x25519_temporary():
                    self.secure_write("✅ Cleared temporary x25519 key material\n")
                else:
