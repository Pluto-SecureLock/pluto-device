...
# Modes
AES_ENCRYPT = const(0x00)
AES_DECRYPT = const(0x01)
...
class ATECC:
    ...
    def aes_encrypt(self, slot: int, plaintext: bytes) -> bytes:
        assert len(plaintext) == AES_BLOCK_SIZE, "Plaintext must be exactly 16 bytes."
        self.wakeup()
        param2 = (slot << 8) | 0x00  # slot in upper byte, mode=0 (ECB)
        self._send_command(OP_AES, AES_ENCRYPT, param2, plaintext)
        time.sleep(0.05)
    ...
    def aes_decrypt(self, slot: int, ciphertext: bytes) -> bytes:
        assert len(ciphertext) == AES_BLOCK_SIZE, "Ciphertext must be exactly 16 bytes."
        self.wakeup()
        param2 = (slot << 8) | 0x00  # mode 0 (ECB)
        self._send_command(OP_AES, AES_DECRYPT, param2, ciphertext)
        time.sleep(0.05)
        response = bytearray(16)
        self._get_response(response)
        self.idle()
    ...
    def write_aes_key(self, slot: int, key: bytes):
        """Write 32-byte AES key to given slot"""
        assert len(key) == 32
        self.wakeup()
        zone = 0x02 | 0x80  # Data zone, 32-byte write
        address = slot * 2  # 32-byte slot = 2 words
        self._send_command(OP_WRITE, zone, address, key)
        time.sleep(0.05)
        status = bytearray(1)
        self._get_response(status)
        print("Key write status:", status[0])
        self.idle()
    ...
    def read_full_config_zone(self) -> bytes:
        """
        Reads the full 128-byte configuration zone.
        Returns: bytes of length 128
        """
        config = bytearray()
        for block in range(4):  # 32 bytes * 4 = 128 bytes
            buf = bytearray(32)
            self._read(zone=0x00 | 0x80, address=block * 8, buffer=buf)
            config.extend(buf)
        return bytes(config)
    ...
    def _get_response(self, buf: Sized, length: int = None, retries: int = 20) -> int:
        ...
        if length is None:
            length = len(buf)
        response = bytearray(length + 3)  # 1 byte header, len bytes data, 2 CRC
        with self._i2c_device as i2c:
        ...
        # Debug print
        print(f"↩️  Raw response: {[hex(b) for b in response]}")

        # CRC check
        crc_recv = response[-2] | (response[-1] << 8)
        crc_calc = self._at_crc(response[0:-2])
        if crc_recv != crc_calc:
            raise RuntimeError(f"CRC Mismatch! recv={hex(crc_recv)} calc={hex(crc_calc)}")
